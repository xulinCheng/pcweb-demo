...
axios.defaults.headers = {
    ...
    'Content-Type': 'application/json; charset=utf-8'
        // 'token': sessionStorage.getItem('token') || '' //显示每个请求都带上token
}
axios.headers = { 'token': sessionStorage.getItem('token') || '' } //这样后台是否可以接受到token？？？

// console.log(axios.defaults.headers, axios.headers)
// 请求超时的时间限制
        ...
        if (error && error.response) {
            switch (error.response.status) {
                ...
                case 401:
                    ...
                    // this.router.replace({
                    //     path: '/',
                    //     query: { redirect: this.router.currentRoute.fullPath } //登录成功后跳转入当前浏览的页面
                    // })
                case 403:
                    error.message = '拒绝访问'
                    break
